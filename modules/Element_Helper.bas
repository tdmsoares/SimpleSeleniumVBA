...
Sub RemoveAttribute(ByVal strWebdriverURL As String, ByVal strBrowserSessionId As String, ByVal strElementIdentifierAsCssSelector As String, ByVal strAttrName As String)
    '   OBS: Usar single quotes apostrofo ' no lugar de aspas " para não ter problemas na execução do JS Script
    Call Document_Scripting_Helper.ExecuteSyncScript(strWebdriverURL, strBrowserSessionId, _
                "(document.querySelector('" & strElementIdentifierAsCssSelector & "')).removeAttribute('" & strAttrName & "')")
End Sub